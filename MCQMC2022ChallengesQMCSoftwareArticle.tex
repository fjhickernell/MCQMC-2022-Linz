...
\title*{Challenges in Developing Great QMC Software}
...
\abstract{
Monte Carlo and their often more efficient variant, quasi-Monte Carlo (QMC) methods, have developed since the middle of the last century.  With the explosion in computational science, there is a need for great Monte Carlo and Quasi-Monte Carlo (MCQMC) software to realize the value in MCQMC methods. Based on our observations of what software has been developed to date, and what potential is not yet realized, we propose some criteria for developing great MCQMC software and some practical steps forward.  Our focus is primarily on QMC software.
}
...
Modern programming languages enable a simple Monte Carlo method approximating $\mu := \mathbb{E}[f(\boldsymbol{X})]$, $\boldsymbol{X} \sim \mathcal{U}[0,1]^d$ to be implemented in three lines of code:
\begin{enumerate}
    \item draw $n$ independent identically distributed (IID) standard uniform nodes 
    $$\boldsymbol{x}_1,\dots,\boldsymbol{x}_n \sim \mathcal{U}[0,1]^d,$$
    \item evaluate the integrand $f$ at these nodes to obtain $$y_i := f(\boldsymbol{x}_i), \qquad i=1,\dots,n$$
    ...
    \end{equation}  
\end{enumerate}
However, practitioners may want to utilize advanced sampling methods such as antithetic sampling, importance sampling, or low discrepancy sampling in the case of Quasi-Monte Carlo (QMC) methods. Users may also wish to modify the integrand to accommodate taking the expectation with respect to a non-standard-uniform random variable or to utilize control variates. The above enhancements may require taking the weighted average in place of \eqref{eq:mc_average} to maintain an unbiased estimate. Practitioners may also be interested in determining $n$ adaptively so the approximation is within some error threshold from the true mean $\mu$.
...
\section{Why Develop Software} \label{sec:why_we_need_software}

In 2021, the US Department of Energy held a ``Workshop on the Science of Scientific-Software Development and Use'' \cite{Her19a,ASCR-SSSDU}. This workshop not only discussed diagnostics and treatments for the challenges of developing great scientific software, but also emphasized the importance of implementing theoretical advancements into well written, accessible software libraries. This section describes both the practical and theoretical importance of developing scientific software.

\subsection{Theory to Software}

The chief purpose of most scientific software developments is to make theoretical advancements available to practitioners. Developing theory into practical software leads to algorithms which often require additional considerations. For instance, the numerical stability of a method only becomes relevant when considering how to implement a method onto a finite precision computer. Taking the time to consider such implications and determine relevant algorithm parameters alleviates these burdens form the user. Moreover, the availability of such algorithms enables practitioners with a variety of applications to quickly solve problems. 

\subsection{Software to Theory}

An often overlooked benefit of scientific software  is the feedback garnered for future theoretical development. A few motivating examples are
\begin{itemize}
    \item the early application of QMC to high dimensional option pricing and 
    \item the early application of neural networks to problems in computer vision.
\end{itemize}
The unexpected performance in these examples motivated theoretical developments to justify their success. Moreover, the availability of high quality software will often attract practitioners to a certain methodology over competitors, and in turn may inspire further theoretical development.
