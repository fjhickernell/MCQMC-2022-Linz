...
\begin{itemize}
	\item $g$ may represent the discounted payoff of a financial derivative and $\lambda$ be the probability density function (PDF) for a discretized Brownian motion \cite{Gla03},

	\item  $g$ may represent the velocity of flow at a point in rock and $\lambda$ is the PDF of the porosity field , or

	\item $g\lambda$ is the unnormalized Bayesian posterior density multiplied by parameter of interest.
\end{itemize},
If $\lambda$ is a probability density function for a random variable $\bsT$, then $\mu = \EE[g(\bsT)]$.
