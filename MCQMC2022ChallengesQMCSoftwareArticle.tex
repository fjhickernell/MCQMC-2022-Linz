...
SciPy and PyTorch's original implementations of Sobol' sequences omitted the first point.  The rationale is that the unscrambled first point of the Sobol' sequence is $\bsx_1 = \bszero$, and this point becomes the undesirable $\boldsymbol{-\infty}$ when transformed to mimic a Gaussian distribution.  A vigorous discussion ensued \cite{scipySobol2020a}, which prompted an article by Art Owen \cite{Owe22a} on why one should not drop the first point and destroy the next structure.  Instead one should randomly scramble the Sobol' sequence (and other LD sequences) so that $\bsx_1 \ne \bszero$.  Although SciPy and PyTorch were corrected, other software libraries with LD sequences still omit the first point.

QMC software is sometimes written by those not intimately a part of the QMC community.  Those of us who are most familiar with good QMC practice need to vigilantly be aware of QMC software in popular practices and inform the developers when bugs are spotted.
...
One attribute of great QMC software that often receives insufficient attention is its accessibility.  Developers may write code for their own purposes and move on.  Great software should be stored on a repository, where the latest version can be downloaded.  There should be documentation that guides the user through installation and explains the library's features.  Demos are crucial for showing how the software work and forming a basis for a new user to write code to solve their own examples.  There should be a place for users to report bugs and make feature requests.  Tutorials help newcomers to QMC understand its advantages. Blogs educate a wider audience and enlarge our QMC community.  


\section{Sustainable} \label{CDHS_sec:sustainable} 
Great QMC software should be alive.  It should be under active development or merged into a libraary that is actively maintained.  Commercial software has a infrastructure to sustain the algorithms in their libraries, but without attention, those algorithms can become stale.  Open source software requires a community of active users and developers to keep the library up-to-date and bug-free.  
